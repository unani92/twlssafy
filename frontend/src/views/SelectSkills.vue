  ...
  <div class="selectskills">
    ...
    <div class="selectskills-search">
      ...
      <div class="autocomplete disabled">
        <div @click="searchSkillAdd" style="cursor: pointer" v-for="res in result" :key="res">{{ res }}</div>
      </div>
    </div>
    <div v-if="userSkill.length" class="selectskills-selected">
      <h2>Selected</h2>
      <div class="hotskills">
        <div v-for="skill in userSkill"
             :key="skill"
             :id="skill"
             class="skill-badge">
          <span>{{ skill }}</span>
...
<style scoped>
  ...
  .selectskills-search, .selectskills-main, .selectskills-selected {
    max-width: 530px;
    margin-top: 1rem;
    margin-bottom: 1rem;
    margin-left: auto;
